...
export const FlickeringGrid: React.FC<FlickeringGridProps> = ({
  ...
  const resolveColor = useCallback((colorValue: string | undefined): string => {
    ...
    if (colorValue && !colorValue.startsWith("var(")) {
      return colorValue.startsWith("rgb") ? colorValue.replace(/\)$/, ",") : `${colorValue},`
    }

    // Direct check of document class for light/dark
  ...
  const memoizedColor = useMemo(() => {
    return resolvedColor.endsWith(",") ? resolvedColor : `${resolvedColor},`
  }, [resolvedColor])
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      // Cap at 30 FPS to prevent main thread blocking and save battery/CPU
      if (time - lastTime < 33) {
