      ...
      images: [
        {
          ...
          alt: `${title} - Chrome Extension & Automation Case Study`,
        },
      ],
    },
    twitter: {
...
function extractFaqs(content: string): { question: string; answer: string }[] {
  ...
  // Pattern 1: <FaqCard question="..." answer="..." />
  const faqCardRegex = /<FaqCard\s+question="([^"]+)"\s+answer="([^"]+)"/g;
  let match;
  while ((match = faqCardRegex.exec(content)) !== null) {
    faqs.push({ question: match[1].trim(), answer: match[2].trim() });
  }

  // Pattern 2: Accordion with question="..."
  const genericFaqRegex = /question="([^"]+)"[\s\S]*?<AccordionContent[^>]*>([\s\S]*?)<\/AccordionContent>/g;
  while ((match = genericFaqRegex.exec(content)) !== null) {
  ...
  // Pattern 3: <FAQ question="..." answer="..." />
  const faqAttrRegex = /<FAQ\s+question="([^"]+)"\s+answer="([^"]+)"/g;
  while ((match = faqAttrRegex.exec(content)) !== null) {
    const q = match[1].trim();
...
export default async function ProjectPage({
  ...
  // Extract FAQs from content for dynamic FAQPage Schema
  const faqs = extractFaqs(post.content || "");

  const graphEntities: any[] = [
    {
      "@type": "TechArticle",
      "@id": `${DATA.url}/projects/${slug}#article`,
      headline: post.title,
      datePublished: post.publishedAt,
      dateModified: post.updatedAt || post.publishedAt,
      description: post.summary,
      inLanguage: "en-US",
      mainEntityOfPage: `${DATA.url}/projects/${slug}`,
      image: post.image
        ? (post.image.startsWith("http") ? post.image : post.image.startsWith("/") ? `${DATA.url}${post.image}` : `${DATA.url}/${post.image}`)
        : `${DATA.url}/projects/${slug}/opengraph-image`,
      url: `${DATA.url}/projects/${slug}`,
      proficiencyLevel: "Expert",
      dependencies: "Chromium, Manifest V3, WebExtensions API, TypeScript, React",
      author: {
        "@type": "Person",
        "@id": `${DATA.url}/#person`,
        name: DATA.name,
    ...
    },
    {
      ...
      "@id": `${DATA.url}/projects/${slug}#software`,
      name: post.title.split("—")[0].trim(),
      applicationCategory: post.title.toLowerCase().includes("script") || post.title.toLowerCase().includes("sheets")
        ? "BusinessApplication"
        : "BrowserExtension",
      operatingSystem: "Chromium, Google Chrome, Brave, Microsoft Edge, Opera",
      description: post.summary,
      url: `${DATA.url}/projects/${slug}`,
      offers: {
        "@type": "Offer",
    ...
    {
      "@type": "BreadcrumbList",
      "@id": `${DATA.url}/projects/${slug}#breadcrumb`,
      itemListElement: [
        {
          "@type": "ListItem",
          position: 1,
